...
import { ledgerViews } from "@/components/console-parts";
...
// /ops/ledger: the books themselves, beside the console that watches them work.
//
...
export default async function LedgerPage({ searchParams }: { searchParams: Promise<Query> }) {
  ...
  // The ledger's own four views, listed under the Ledger entry of the sidebar and nowhere else
  // (Yoann's decision of 2026-09-09); the list itself lives in components/console-parts.tsx. The
  // entry count that used to sit beside "Entries" is gone: a count in the navigation is a call to
  // act, and nobody acts on 36 postings (decision 3).
  const views = ledgerViews(view, { query });
