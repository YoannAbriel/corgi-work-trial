...
| F-B2-01 | HIGH | CGP-01061 bound on a locally fabricated payment in the shared database; ledger cash exceeds what Stripe holds | Reversal entries plus dated correction event (Yoann's decision); fabricated events confined to corgi_test | DECIDED, fix in B8a after B5 merge |
| F-B2-02 | MED | A delivery arriving while another is in flight gets 200, so a killed function can leave an event stuck with Stripe no longer retrying; no stuck-events view | 503 while a lease is in flight; stuck and ignored events view with replay | OPEN: 503 in B5 branch; view in B13 |
| F-B2-03 | MED | Expired Checkout Session leaves the draft unpayable | checkout.session.expired marks failed; new operation and key per attempt | DECIDED, in B5 branch |
| F-B2-04 | MED | The only real payment used a session created by the delegate's local server; no session created by the deployed app yet | Yoann creates and pays a fresh draft on the deployed URL | OPEN (Yoann, 3 minutes) |
