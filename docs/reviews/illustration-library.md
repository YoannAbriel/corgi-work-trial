...
- The same audit downsampled each Spark PNG with Lanczos to its original dimensions, then calculated absolute per-channel differences on signed 16-bit arrays: mean range **0.196893–0.369783**, maximum 99th-percentile difference **3**. This independently reproduces the PROGRESS claim. Repeated the comparison using the **delivered WebPs**, including lossy compression: mean range **0.729019–0.853973** on the 0–255 scale. No source/composition mismatch was found. These are fidelity measurements, not a claim of new generative detail or lossless WebP encoding.
