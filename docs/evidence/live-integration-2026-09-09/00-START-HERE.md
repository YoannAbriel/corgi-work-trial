...
- For sharing after review, upload this folder to the candidate's chosen file-sharing service, grant reviewers view-only access, and verify the resulting link from a viewer session. Dashboard deep links in the manifest still require the existing Stripe account access; they do not grant it.
