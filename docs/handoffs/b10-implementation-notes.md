...
Migration 0013 stores the DATE OF THE COMPARED RECORD on each item: the provider's created time,
...
written before 0013 use their first-seen instant instead, which is inside the window of the run
...
1. `db/migrations/0011_reconciliation.sql` - the two tables, their guards, their grants, and the
   ...
   `db/migrations/0013_reconciliation_item_record_date.sql` - the record date each item carries,
