...
Before submission, rehearse the core money path and hostile cases using actual code. Yoann should explain the important lines in his own words, including transaction boundaries, idempotency, money arithmetic, eligibility and approval checks. Reduce optional code that cannot be understood within the remaining time; do not remove mandatory behavior or hide a gap. Track 1 is selected; stack and provider decisions remain open.
