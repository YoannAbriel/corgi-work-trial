...
Read `READABLE-CODE.md`. Flag opaque abstractions, hidden side effects and financial formulas without a checkable example. A reviewer can assess explainability but cannot certify Yoann's understanding on his behalf. Keep candidate walkthrough status separate from the technical verdict.

## Automatic-fail review gate

Read [AUTOMATIC-FAILS.md](AUTOMATIC-FAILS.md) in full. For the assigned scope, map AF-01 through AF-06 to actual controls and evidence; mark final-delivery checks that are not yet possible as NOT RUN. Inspect SQL/ORM and privileged repair paths, actual provider modes, honest live/simulated labels, staged/history secret-scan evidence and candidate walkthrough gaps. Do not let a scoped design PASS imply that the six delivery gates passed. No reviewer can waive a disqualification rule or certify that remediation erases a past violation.
