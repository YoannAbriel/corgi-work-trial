...
import { correctEndorsementDateMoney, correctionFormulaLines } from "./correction";
...
test("entered at day 130 and corrected to day 100: the customer owes 5047 more", () => {
  const correction = correctEndorsementDateMoney(bookedOnDay130, "2028-06-09");
...
test("entered at day 100 and corrected to day 130: the customer is owed 5047 back", () => {
  const correction = correctEndorsementDateMoney(bookedOnDay100, "2028-07-09");
...
test("a difference above $500 needs the customer's approval, one above $1,000 out needs an approver", () => {
  ...
  const collectALot = correctEndorsementDateMoney(bookedLate, "2028-03-01");
  ...
  const refundALot = correctEndorsementDateMoney(bigRise, "2029-02-01");
...
test("correcting to the same day the endorsement already covers moves no money", () => {
  ...
  const correction = correctEndorsementDateMoney(bookedOnDay100, "2028-06-09");
...
test("a date outside the term is refused, not priced", () => {
  assert.throws(() => correctEndorsementDateMoney(bookedOnDay130, "2028-02-29"), EndorsementNotComputable);
  assert.throws(() => correctEndorsementDateMoney(bookedOnDay130, "2029-03-02"), EndorsementNotComputable);
});
...
test("the difference is always what the ledger will hold: after minus before, both ways round", () => {
  for (const wrongDate of ["2028-03-15", "2028-06-09", "2028-07-09", "2028-11-01"]) {
    for (const rightDate of ["2028-03-15", "2028-06-09", "2028-07-09", "2028-11-01"]) {
      const correction = correctEndorsementDateMoney(endorsementOn(wrongDate), rightDate);
      assert.equal(
